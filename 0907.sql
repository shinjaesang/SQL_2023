...
--고객의 이름과 주문한 도서의이름을 구하시오
-- 3개의 테이블에 대한 동등조인
select name, bookname from customer c, book b, orders o
where c.custid = o.custid and o.bookid = b.bookid
order by name;

--도서 가격이 16000원인 도서를 주문한 고객이름과 도서이름을 출력
select name, bookname 
from customer c, book b, orders o
where c.custid = o.custid and o.bookid = b.bookid and price>=10000;

-- 외부조인(Outer Join)
-- 동등조인을 하게되면 갑이 동등하지 않은 행은 무시되지만 외부조인을 하게되면 기준이 되는 테이블의 값이 동등하지 않은 데이터 행도 포함해서 출력해준다
...
--웅진지식하우스에서 출판한 도서를 구매한 고객의 이름을 출력
select name
from customer;
